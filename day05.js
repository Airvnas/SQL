...
db.orders.find()
//orders에서 고객id와 주문일 주문가격 가져와 출력
db.orders.aggregate([{$project:{cust_id:1,ord_date:1,price:1}}])

//orders에서 총 주문건수를 가져와 출력
db.orders.aggregate([{
    $group:{
        _id:null,
        count:{$sum:1}
...
//$sum:1 의 의미는 document 1개당 1을 더해라
db.orders.aggregate([{
    $group:{
        _id:'$cust_id',
        count:{$sum:1}
...
//$sort:{필드명:1} :1값 오름차순 , -1값 내림차순
//고객별 최소 주문가격(price)과 최대 주문가격
db.orders.aggregate([{
    $group:{
        _id:'$cust_id',
        maxPrice:{$max:"$price"},
...
//select min(price), max(price) from orders group by cust_id
db.orders.aggregate([{
    $group:{
        _id:'$cust_id',
        avgPrice:{$avg:"$price"}
...
//[4]고객별 주문 건수가 1개를 초과하는 주문데이터를 보여주새요
...
//[6] status별 주문 총액 가져와 출력
db.orders.aggregate([{
    $group:{
        _id:"$status",sumPrice:{$sum:"$price"}
   }
},
   {$sort:{sumPrice:-1}}])

//[7]status가 'A'인 주문을 대상으로 고객별 주문 총액
db.orders.aggregate([{
   $match:{status:'A'}
...
db.items.find()
//{$multiply:["$price",$quantity]} ==>price*qty
db.items.aggregate([{
    $group:{_id:"$item",totalPrice:{$sum:{$multiply:["$price","$quantity"]}}}
}])

//[11] 일자별 총 주문 금액,평균 수량, 총 주문건수를 출력
db.items.aggregate([{
